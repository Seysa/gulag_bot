...
module.exports = {
	...
	async execute(client, message, args) {
		...
		if (args[0]) {
			...
			// is an ID
			if (channel) {
				await message.reply(`Goulag channel is now '${channel.name}'`);
				cfg.writeServerGoulagId(message.guild.id, args[0]);
			}
			else {
				return await message.reply(`Couldn't identify voice channel id`);
			}
		}
		else {
			...
			if (voiceId) {
				...
				return await message.channel.send(`Goulag channel set at your location (` + userInGuild.voice.channel.name + `)`);
			}
			else {
